...
class IndexTemplateView(AdminDashBoardMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[
            'personal_training_sessions'] = PersonalTraining.objects.filter(
...
def toggle_user_suspension(request, pk):
    ...
    if user.role == 'Member':
        return HttpResponseRedirect(
            reverse("dashboard:members_dashboard:member_details",
                    kwargs={"pk": user.id}))
    elif user.role == 'Trainer':
        return HttpResponseRedirect(
            reverse("dashboard:trainers_dashboard:trainer_details",
                    kwargs={"pk": user.id}))
